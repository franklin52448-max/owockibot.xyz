...
async function atomicClaim(id, claimerAddress) {
  ...
  if (!isNaN(numId) && SUPABASE_KEY) {
    ...
    // BUG FIX #2: Self-dealing check inside atomic path too
    if (existing.creator && existing.creator.toLowerCase() === claimerAddress.toLowerCase()) {
      console.log(`[ATOMIC CLAIM] Self-dealing blocked for bounty ${id}`);
      return null;
    }
  ...
  // BUG FIX #2: Self-dealing check in fallback path too
  if (bounty.creator && bounty.creator.toLowerCase() === claimerAddress.toLowerCase()) return null;
...
app.get('/bounties/:id', async (req, res) => {
  ...
  // BUG FIX #2: Auto-fix self-dealing on read (lazy cleanup)
  if (
    bounty.status === 'claimed' &&
    bounty.creator && bounty.claimedBy &&
    bounty.creator.toLowerCase() === bounty.claimedBy.toLowerCase()
  ) {
    console.log(`[SELF-DEAL LAZY FIX] Bounty #${bounty.id} detected on read — releasing`);
    bounty.status = 'open';
    bounty.claimedBy = null;
    bounty.claimedAt = null;
    bounty.submissions = [];
    bounty.feedback = (bounty.feedback || '') + ' [auto-released: self-dealing detected on read]';
    bounty.updatedAt = Date.now();
...
// ============ BUG FIX #1: COMMENT AUTH ============
// Comments now require a signed message to prevent impersonation.
// The client signs: "comment:{bountyId}:{timestamp}" and sends the signature.
// Server verifies the signer matches the claimed author_address.

/**
 * Verify a comment signature — proves wallet ownership.
 * The message format is: "comment:{bountyId}:{timestamp}"
 */
function verifyCommentSignature(authorAddress, bountyId, timestamp, signature) {
  try {
    if (!authorAddress || !bountyId || !timestamp || !signature) return false;
    const message = `comment:${bountyId}:${timestamp}`;
    const recovered = ethers.verifyMessage(message, signature);
    return recovered.toLowerCase() === authorAddress.toLowerCase();
  } catch (err) {
    console.error('[COMMENT AUTH] Signature verification failed:', err.message);
    return false;
  }
}

// ============ BUG FIX #3: CONSISTENT COMMENT GATING ============
// Unified rule: comments are gated on completed/cancelled bounties (invite-only).
// The gate is applied consistently regardless of bounty ID.
function isCommentAllowed(bounty) {
  // Open / claimed / submitted bounties: comments allowed
  if (['open', 'claimed', 'submitted'].includes(bounty.status)) return true;
  // Completed / cancelled bounties: invite-only (only creator, claimer, or mods can comment)
  return false;
}

/**
 * Post a comment on a bounty (requires cryptographic signature)
 * POST /bounties/:id/comment
 *
 * Body: { address, text, timestamp, signature }
 * - address: the commenter's wallet
 * - text: comment content (max 2000 chars)
 * - timestamp: Date.now() used when signing (must be within 5 min)
 * - signature: ethers.signMessage("comment:{bountyId}:{timestamp}")
 */
app.post('/bounties/:id/comment', async (req, res) => {
  const { address, text, timestamp, signature } = req.body;
  ...
  if (!timestamp || !signature) {
    return res.status(400).json({ error: 'timestamp and signature required (sign "comment:{bountyId}:{timestamp}")' });
  }

  // Anti-replay: reject signatures older than 5 minutes
  const SIGNATURE_MAX_AGE_MS = 5 * 60 * 1000;
  if (Math.abs(Date.now() - Number(timestamp)) > SIGNATURE_MAX_AGE_MS) {
    return res.status(400).json({ error: 'Signature expired. Please sign a fresh message.' });
  }

  // Payload size check
  if (text.length > 2000) {
    return res.status(413).json({ error: 'Comment too long (max 2000 characters)' });
  }
  ...
  // BUG FIX #3: Consistent invite-only gate
  if (!isCommentAllowed(bounty)) {
    const caller = address.toLowerCase();
    const isCreator = bounty.creator?.toLowerCase() === caller;
    const isClaimer = bounty.claimedBy?.toLowerCase() === caller;
    const isCallerMod = isMod(address);
    if (!isCreator && !isClaimer && !isCallerMod) {
      return res.status(403).json({ error: 'Invite-only: comments restricted on completed/cancelled bounties' });
    }
  }

  // BUG FIX #1: Verify cryptographic signature (prevents impersonation)
  const validSig = verifyCommentSignature(address, req.params.id, timestamp, signature);
  if (!validSig) {
    console.log(`[COMMENT AUTH REJECTED] Invalid signature from ${address} on bounty ${req.params.id}`);
    return res.status(403).json({ 
      error: 'Invalid signature. You must sign "comment:{bountyId}:{timestamp}" with your wallet to prove ownership.' 
    });
  }

  // Add comment
  if (!bounty.comments) bounty.comments = [];
  ...
  const updated = await updateBounty(bounty.id, bounty);
  console.log(`[COMMENT] ${address} commented on bounty ${req.params.id}`);
  res.json({ success: true, comment: bounty.comments[bounty.comments.length - 1] });
});
...
// ============ BUG FIX #2: SELF-DEALING AUTO-CANCEL ============
// Scans for bounties where creator === claimer and auto-releases them.
// Runs on startup and every 15 minutes to catch any that slipped through.
async function scanAndCancelSelfDealing() {
  try {
    const allBounties = await getAllBounties();
    let fixed = 0;
    for (const bounty of allBounties) {
      if (
        ...
        console.log(`[SELF-DEAL AUTO-CANCEL] Bounty #${bounty.id} — creator === claimer (${bounty.creator}). Releasing.`);
        bounty.status = 'open';
        bounty.claimedBy = null;
        bounty.claimedAt = null;
        bounty.submissions = [];
        bounty.feedback = bounty.feedback
          ? bounty.feedback + ' [auto-released: self-dealing detected]'
          : 'Self-dealing detected: creator and claimer are the same wallet. Bounty released.';
        bounty.updatedAt = Date.now();
        await updateBounty(bounty.id, bounty);
        fixed++;
      }
    ...
    if (fixed > 0) {
      console.log(`[SELF-DEAL SCAN] Released ${fixed} self-dealing bounties`);
    }
  } catch (err) {
    console.error('[SELF-DEAL SCAN] Error:', err.message);
  }
}

// Run on startup (after DB is ready) and every 15 minutes
scanAndCancelSelfDealing();
setInterval(scanAndCancelSelfDealing, 15 * 60 * 1000);
